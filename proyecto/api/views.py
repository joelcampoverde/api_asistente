...
def process_command(request):
    ...
    if not Usuario.objects.filter(id=usuario.id).exists():
        Usuario.objects.create(id=usuario.id, email=usuario.email, nombre=usuario.first_name)

    # Crear o vincular la agenda al usuario
    ...
    if "agregar" in command:
        ...
        if not (descripcion and fecha_inicio and fecha_fin):
            return Response({"response": "Faltan datos. Asegúrate de incluir descripción, fecha de inicio, y fecha de fin."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    # Proceso para consultar eventos
        ...
        eventos = Evento.objects.filter(agenda=agenda)
        if not eventos:
    ...
    # Proceso para eliminar un evento
        ...
        evento = Evento.objects.filter(agenda=agenda, descripcion=evento_descripcion).first()
...
# from rest_framework.permissions import IsAuthenticated
# from rest_framework_simplejwt.authentication import JWTAuthentication
# from .models import Evento, Agenda, TipoEvento
# from django.utils.dateparse import parse_datetime
# import uuid
...
#         # Validar el usuario autenticado a través del token
#         usuario = request.user  # Obtiene el usuario autenticado a partir del token
